...
public:
  ...
  Boolean contains(const K &k) const { THROW("tbd"); }
...
public:
  ...
  Mutable<V> mutable_get(const K &k) override {
    ...
    if (!value.has_value()) {
      if (factory_fn_.has_value()) {
        ...
        THROW("No value found, and no factory.");
      }
    }
  ...
  Shared<ConstIterator<KV<K, V>>> iterator() const override { THROW("TBD"); }
  Void erase(const K &k) override { THROW("TBD"); }
